...
function submitTranscription(id) {
    var variables = $("#form" + id).serialize();
    
    $("#block" + id).hide("slow");
    
    $.post("processTranscription.php", variables, function (response) {
	    $("#notifications").html("Transcription saved!  Thanks!");
	});
}

function addEmail() {
    var variables = $("#emailForm").serialize();
    
    $.post("addEmail.php", variables, function (response) {
	    $("#notifications").html("We added you to the list.  Thanks!");
	});
}
